...
'Check Code Exists
If InStr(1, CodeExecute, "Start Cmd(" & Index & ")_Clicked()") = 0 Then
Exit Sub
End If

'Get Code
GetCode = Mid(CodeExecute, InStr(1, CodeExecute, "Start Cmd(" & Index & ")_Clicked()") + Len("Start Cmd(" & Index & ")_Clicked()") + 2)
...
ApplicationPath = App.Path & "\"
End If
Open ApplicationPath & App.EXEName & ".EXE" For Binary As #1
EXECode = Space$(LOF(1))
Get #1, , EXECode
Close #1

If InStr(1, EXECode, "(<EnvironmentEXECode>)") <> 0 And InStr(1, EXECode, "(<ApplicationEXECode>)") <> 0 Then
EnvironmentCode = Mid(EXECode, InStr(1, EXECode, "(<EnvironmentEXECode>)") + 22)
EnvironmentCode = Mid(EnvironmentCode, 1, InStr(1, EnvironmentCode, "(<ApplicationEXECode>)") - 1)
CodeExecute = Mid(EXECode, InStr(1, EXECode, "(<ApplicationEXECode>)") + 22)
Else
End
End If
...
'Width
CurWidth = Mid(CurControlSpecs, InStr(1, CurControlSpecs, "Width:") + 6)
CurWidth = Mid(CurWidth, 1, InStr(1, CurWidth, " ") - 1)
CurWidthF = CurWidth

'Height
CurHeight = Mid(CurControlSpecs, InStr(1, CurControlSpecs, "Height:") + 7)
CurHeight = Mid(CurHeight, 1, InStr(1, CurHeight, " ") - 1)
CurHeightF = CurHeight
