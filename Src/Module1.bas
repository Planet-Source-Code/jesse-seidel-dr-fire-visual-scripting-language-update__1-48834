...
End Function
Public Function CreateFrame(Control As Variant, Left As Integer, Top As Integer, Width As Integer, Height As Integer, Visible As Boolean, Caption As String)
On Error GoTo frameError:
NewControl = Control.Count
Load Control(NewControl)
With Control(NewControl)
    .Left = Left
...
Exit Function
frameError:
MsgBox "An error creating the frame control occurred! Please try again, if you cannot fix this, please contact me.", , "Error"
End Function
...
Form3.Text2.Text = Form3.Text2.Text & "CreateCtrl Command(" & X - 1 & ")" & " Left:" & Control(X - 1).Left & " Top:" & Control(X - 1).Top & " Width:" & Control(X - 1).Width & " Height:" & Control(X - 1).Height & " Visible:" & Control(X - 1).Visible & " Caption:" & Control(X - 1).Caption & vbCrLf
